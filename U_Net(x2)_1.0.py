...
def unet_model(n_classes=255, im_sz=128, n_channels=7, n_filters_start=32, growth_factor=2, upconv=True,
    ...
    model.compile(optimizer=Adam(), loss=weighted_binary_crossentropy,metrics=[mean_iou])
    return model
...
checkpointer = ModelCheckpoint('good_model_2.h5', verbose=1, save_best_only=True)
results = model.fit(X_train, y_train, validation_split=0.1, batch_size=16, epochs=1000, 
...
model = load_model('good_model_2.h5', custom_objects={'mean_iou': mean_iou})
